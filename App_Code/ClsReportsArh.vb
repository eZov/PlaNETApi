...
WHERE (r.report_tree_name ='plc_net_arh')
UNION 
SELECT 
	rg.id AS Id, 
...
WHERE rg.report_tree_name ='plc_net_arh';
]]>.Value
...
ORDER BY rd2p.`report_data_parameters_code` ;
            ...
            If ParamListValues IsNot Nothing AndAlso _paramNum = ParamListValues.Count Then
                ...
                Select Case ParamListValues.Count
                    Case 1
                        _parametri = ParamListValues.Item(0)
                    Case 2
                        _parametri = ParamListValues.Item(0) + "," + ParamListValues.Item(1)
                    Case Else
...
Call {nazivProcedure}({parametri});
                ...
                myda = New MySqlClient.MySqlDataAdapter(strSQL, myconnection)
                myda.Fill(myds)
                'myds.WriteXml(HttpContext.Current.Server.MapPath("~/Data/" + "PorezSpec.xml"), XmlWriteMode.WriteSchema)
...
SELECT report_structure, report_name, report_size FROM sys_net_reports_structure WHERE report_code=@report_code;
            ...
            While rd.Read
                ...
                _fileName = rd.GetString(rd.GetOrdinal("report_name"))
                CurrentRptName = _fileName
...
SELECT report_structure, report_name, report_size FROM sys_net_reports_structure WHERE report_code=@report_code;
    ...
    Public Function ReportExists() As Boolean
        ...
        If File.Exists(HttpContext.Current.Server.MapPath("~/Data/" + _docName)) Then Return True
